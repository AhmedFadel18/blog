...
<body>
   ...
   <div class="container m-auto pt-3 pb-2 text-center">
    <form action="{{ route('post.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="pt-2 pb-2">
            <input class="form-control pt-2" type="text" name="title" value="{{ old('title') }}" id="title" placeholder="Enter title">
            @error('title')
        ...
        <div class="pt-2 pb-2">
            <textarea class="form-control" name="description" value="{{ old('description') }}" id="description" placeholder="Enter your topic"></textarea>
            @error('description')
            <span class="text-danger">{{ $message }}</span>
        @enderror
        </div>
        ...
        <div class="pt-2 pb-2 form-group">
            ...
            <input type="radio" class="form-group" id="{{ $tag->name }}" name="tag" value="{{ $tag->id }}"> ] ,
            @endforeach
        </div>
